    ...
    if request.method == 'POST':
        ...
        if add_manager_form.validate_on_submit():
            ...
            return render_template(
                "manager/addmanager.html",
                headline="Add Managers",
                form=add_manager_form,
                managers=managers)
            ...
            if del_form.validate_on_submit():
                ...
                for i in delete_id_list:
                    ...
                    itemToDelete = db.session.query(
                        Manager).filter(Manager.ManagerId == i)
                    itemToDelete.delete()
                    db.session.commit()
                    print("deleted data with id " + i)

                managers = session.query(Manager).order_by(
                    Manager.ManagerId).all()
                return render_template(
                    "manager/viewmanagers.html",
                    managers=managers,
                    headline="Managers",
                    form=del_form)
            else:
                print("invalide Form")
                return render_template(
                    "manager/deletemanager.html",
                    managers=managers,
                    headline="Delete Managers",
                    form=del_form)
...
def manager_edit():
    ...
    item_to_edit = db.session.query(Manager).filter(
        Manager.ManagerId == edit_manager_id).first()
    if request.method == 'POST':
        print("Post")
        ...
        return render_template(
            "manager/editmanager.html",
            headline="Edit Managers",
            form=edit_manager_form)
